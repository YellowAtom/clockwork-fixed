...
--	local colorWhite = Clockwork.option:GetColor("white")
	...
	for k, v in pairs(self.storedList) do
		local alpha = math.Clamp(Clockwork.kernel:CalculateAlphaFromDistance(512, eyePos, v.position) * 1.5, 0, 255)

		if alpha > 0 then
			if not v.markupObject then
				v.markupObject = markup.Parse("<font=" .. large3D2DFont .. ">" .. string.gsub(v.text, "\\n", "\n") .. "</font>")
