...
function ENT:Initialize()
	...
	ent:SetModel("models/ug_imports/cassette/casette.mdl")
	ent:SetNoDraw(true)
...
function ENT:GetDisplayPos()
	...
	local pos = self:GetPos() + ang:Right() * -2.0625 + ang:Up() * 10 + ang:Forward() * -2.2
...
function ENT:GetButtonData()
	...
	for _, btn in pairs(self.data.buttons) do
		...
		table.insert(data, {
			...
			height = height,
			button = btn,
			hasfocus = cursorPos and (cursorPos.x >= x and cursorPos.x <= x + width) and (cursorPos.y >= y and cursorPos.y <= y + height)
		})
	end
...
function ENT:DrawDisplay()
	...
	local barHeight = fontHeight + display.btn_padding * 2 + display.btn_inr_padding * 2
