...
function cwAnimatedLegs:ShouldDrawLegs()
	return IsValid(self.LegsEntity) and Clockwork.Client:Alive() and self:CheckDrawVehicle() and GetViewEntity() == Clockwork.Client and not Clockwork.Client:ShouldDrawLocalPlayer() and not IsValid(Clockwork.Client:GetObserverTarget())
end

-- A function to check if a vehicle should be drawn.
function cwAnimatedLegs:CheckDrawVehicle()
	return Clockwork.Client:InVehicle() and (Clockwork.Client:GetVehicle() and Clockwork.Client:GetVehicle():GetThirdPersonMode()) or not Clockwork.Client:InVehicle()
end
...
function cwAnimatedLegs:WeaponChanged(weapon)
	if IsValid(self.LegsEntity) then
		...
		if not Clockwork.Client:InVehicle() then
			if (self.HoldType ~= "fist" or not Clockwork.player:GetWeaponRaised(Clockwork.Client)) and self.BoneHoldTypes[self.HoldType] then
				self.BonesToRemove = self.BoneHoldTypes[self.HoldType]
			else
				self.BonesToRemove = self.BoneHoldTypes["default"]
			end
		elseif not Clockwork.entity:IsChairEntity(Clockwork.Client:GetVehicle()) then
