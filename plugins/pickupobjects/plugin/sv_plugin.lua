...
function cwPickupObjects:ForceDropEntity(player)
	...
	if IsValid(entity) then
		...
		if Clockwork.config:Get("prop_kill_protection"):Get() and entity.cwLastCollideGroup then
...
function cwPickupObjects:ForcePickup(player, entity, trace)
	...
	if Clockwork.config:Get("prop_kill_protection"):Get() and not entity.cwLastCollideGroup then
...
--	local curTime = CurTime()
	...
	if IsValid(entity) and IsValid(holdingGrab) and player:Alive() and not player:IsRagdolled() then
