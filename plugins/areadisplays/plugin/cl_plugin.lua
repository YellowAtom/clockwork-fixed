...
Clockwork.datastream:Hook("AreaAdd", function(data)
	if not cwAreaDisplays:HasExpired(data) then
...
Clockwork.datastream:Hook("AreaRemove", function(data)
	for k, v in pairs(cwAreaDisplays.storedList) do
		if v.name == data.name and v.minimum == data.minimum and v.maximum == data.maximum then
...
function cwAreaDisplays:AddAreaDisplayDisplay(areaTable)
	areaTable.name = string.Replace(areaTable.name, "%t", Clockwork.time:GetString())

	if not areaTable.class then
	...
	if not self.activeDisplays[uniqueID] then
...
function cwAreaDisplays:CalculateDisplayAlpha(displayInfo, index)
	...
	elseif CurTime() >= displayInfo.goBackTime then
		if not displayInfo.fadeTime then
...
function cwAreaDisplays:HandleAreaTable(areaTable, index)
	...
	if not areaTable.doesExpire then
		self.currentAreaDisplay = areaTable.name
		Clockwork.plugin:Call("PlayerEnteredArea", areaTable.name, areaTable.minimum, areaTable.maximum)
		bCalledHooks = true
	end
	...
	if bCalledHooks then return true end
end
...
function cwAreaDisplays:DrawDisplay3D(displayInfo)
	...
	Clockwork.kernel:DrawInfo(areaTable.name, 0, 0, colorWhite, displayInfo.alpha, nil, function(x, y, width, height) return x, y - height / 2 end, 3)
	cam.End3D2D()
	cam.End3D()
...
function cwAreaDisplays:DrawDisplayScrolling(displayInfo, info)
	...
	if not displayInfo.scrollInfo then
	...
	if bIsGoingBack and not displayInfo.scrollInfo.isGoingBack then
		displayInfo.scrollInfo.isGoingBack = true
		displayInfo.scrollInfo.index = 0
	end

	if not displayInfo.scrollInfo.nextType or CurTime() >= displayInfo.scrollInfo.nextType then
		...
		if displayInfo.scrollInfo.isGoingBack then
			displayInfo.scrollInfo.text = string.utf8sub(areaTable.name, displayInfo.scrollInfo.index + 1)
		else
			displayInfo.scrollInfo.text = string.utf8sub(areaTable.name, 0, displayInfo.scrollInfo.index)
		end
	...
	local defaultWidth, _ = Clockwork.kernel:GetCachedTextSize(introTinyTextFont, string.upper(areaTable.name))
	local scrollWidth, _ = Clockwork.kernel:GetCachedTextSize(introTinyTextFont, string.upper(displayInfo.scrollInfo.text))
	local sNextCharacter = ""
	local newX = info.x

	if displayInfo.scrollInfo.isGoingBack then
		sNextCharacter = string.utf8sub(areaTable.name, displayInfo.scrollInfo.index, displayInfo.scrollInfo.index)
--		local _, textWidth = Clockwork.kernel:DrawInfo(string.upper(sNextCharacter), info.x, info.y, informationColor, math.max(displayInfo.alpha - 25, 0), true, function(x, y, width, height) return x + (defaultWidth - scrollWidth) - width, y end)
		newX = newX + (defaultWidth - scrollWidth)
	else
		sNextCharacter = string.utf8sub(areaTable.name, displayInfo.scrollInfo.index + 1, displayInfo.scrollInfo.index + 1)
	end

	local newY, textWidth = Clockwork.kernel:DrawInfo(string.upper(displayInfo.scrollInfo.text), newX, info.y, colorWhite, displayInfo.alpha, true)

	if not displayInfo.scrollInfo.isGoingBack and sNextCharacter ~= "" then
		Clockwork.kernel:DrawInfo(string.upper(sNextCharacter), newX + textWidth, info.y, informationColor, math.max(displayInfo.alpha - 25, 0), true)
	end
...
function cwAreaDisplays:HasExpired(areaDisplay)
	if areaDisplay and areaDisplay.doesExpire then
		...
		if self.expiredList[position] == areaDisplay.name then return true end
	end
