...
function cwStamina:PostPlayerSpawn(player, lightSpawn, changeClass, firstSpawn)
	if not firstSpawn and not lightSpawn then
...
function cwStamina:PlayerCanThrowPunch(player)
	if player:GetCharacterData("Stamina") <= 10 then return false end
end
...
function cwStamina:PlayerThink(player, curTime, infoTable)
	...
	if not player:IsNoClipping() and player:IsOnGround() then
	...
	if stamina < 30 and Clockwork.event:CanRun("sounds", "breathing") then
		bPlayerBreathSnd = false
	end

	if not player.nextBreathingSound or curTime >= player.nextBreathingSound then
		if not Clockwork.player:IsNoClipping(player) then
