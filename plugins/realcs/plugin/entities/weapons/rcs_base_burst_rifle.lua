...
function SWEP:PrimaryAttack()
	...
	--only does this stuff if the gun is Bursting 
	if self:GetNWInt("FBurst") == 1 then
		...
		self:SetNextSecondaryFire(CurTime() + self.BurstDelay)
		self:EmitSound(self.BurstSound)
		self:ConeStuff()
		...
		self:SetNextPrimaryFire(CurTime() + self.Primary.Delay) --ughhhh
...
--the attack function for the second and third shots for burst fire mode
...
function SWEP:RCSAttack2()
	...
	--only does this if it isnt in burst fire
	if self:GetNWInt("FBurst") == 1 then
		self.FamasBurst = false --makes it so it isnt in burstfire
		self:SetNWInt("FBurst", 0)
		owner:PrintMessage(HUD_PRINTCENTER, "Switched to Automatic")
		self:SetNextSecondaryFire(CurTime() + 0.25)
	else
		self.FamasBurst = true --makes it so it IS in burst fire 
		self:SetNWInt("FBurst", 1)
		owner:PrintMessage(HUD_PRINTCENTER, "Switched to Burst-Fire mode")
		self:SetNextSecondaryFire(CurTime() + 0.25)
	end
