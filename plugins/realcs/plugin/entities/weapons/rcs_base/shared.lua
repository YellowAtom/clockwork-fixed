...
function SWEP:RCSIronsights(sighted)
	if self.Weapon:GetNetworkedInt("deploydelay") > CurTime() or self.canzoom == false or self.ShootafterTakeout > CurTime() or self.Zoom == 1 or self.Zoom == 2 or not self.IronSightsPos then return end
...
function SWEP:Reload()
	...
	if self.ShootafterTakeout > CurTime() or self.Reloadaftershoot > CurTime() or isreloading or self.Weapon:Clip1() >= self.Primary.ClipSize then return end
	self.Weapon:GetOwner():SetFOV(0, 0.3)
	...
	if self.IsShotgun then
		if self.Weapon:GetNetworkedBool("reloading", false) or sgunstartreload then return end
...
function SWEP:Think()
	...
	if self.Owner:KeyDown(bit.bor(IN_FORWARD, IN_BACK, IN_MOVELEFT, IN_MOVERIGHT)) and self.Owner:KeyDown(IN_SPEED) then
		...
		if self.ShootafterTakeout <= CurTime() and deployDELAY <= CurTime() then
			--ironsights
			if self.Owner:KeyPressed(IN_USE) or self.Owner:KeyDown(IN_USE) and self.Weapon:GetNetworkedBool("Ironsights") == false then
...
function SWEP:Holster()
	...
	if self.Owner and self.Weapon and self.Owner:GetFOV() ~= 0 then
...
function SWEP:ShootEffects()
	...
	if not self.Weapon:GetNetworkedBool("Silenced") then
...
if CLIENT then
	...
	function SWEP:DrawHUD()
		if not self.CrossHairIronsight and self.Weapon:GetNetworkedBool("Ironsights") then
