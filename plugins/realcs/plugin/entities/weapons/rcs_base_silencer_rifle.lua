...
function SWEP:Silence()
	...
	self:SetNextPrimaryFire(CurTime() + self.SilenceTime)
	self:SetNextSecondaryFire(CurTime() + self.SilenceTime)
	self.Reloadaftershoot = CurTime() + self.SilenceTime
	self:SetNWInt("deploydelay", CurTime() + self.SilenceTime)

	if self:Clip1() == 0 then
		timer.Simple(self.SilenceTime, function()
			self:OtherReload()
		end)
	end
end
...
function SWEP:RCSAttack2()
	if not self.AlwaysSilenced then
		self:Silence()
	end
end
