...
function SWEP:PrimaryAttack()
	...
	if (self.IsBurst == true) then --only does this stuff if the gun is Burst
		...
		timer.Simple(0.03, function() self:EmitSound(self.Primary.Sound) end)
		timer.Simple(0.06, function() self:EmitSound(self.Primary.Sound) end)
...
function SWEP:RCSAttack2()
	...
	if self:GetNWInt("Burst") == 0 then
		...
		self.IsBurst = false
		owner:PrintMessage(HUD_PRINTCENTER, "Switched to Semi-Automatic")
		self.Primary.Automatic = false
		self:SetNextSecondaryFire(CurTime() + 0.25)
	end
