...
if CLIENT then
	...
	SWEP.Description = "Accuracy International Arctic Warfare Magnum " .. "sniper rifle (Yeah, I didn't put that in the " .. "name, because it's a really long)"
	SWEP.SlotPos = 1
	SWEP.IconLetter = "r"
...
if CLIENT then
	...
	function SWEP:DrawHUD()
		...
		local st = owner:GetInfo("rcs_scopetype")

		if not st then
			CreateClientConVar("rcs_scopetype", "1", true, false)
		end

		local zoomed_in = self:GetNWInt("Zoom")
...
function SWEP:RCSAttack1()
	if self:GetOwner():GetFOV() == 0 then
		self.Zoom = 0
		self:SetNWInt("Zoom", 0)
	end

	--only does this if its not zoomed
	if self:GetNWInt("Zoom") ~= 0 then
		self.canzoom = false
...
function SWEP:ZoomBackIn()
	...
	if self.DidntSwitch == false then
		...
		self.canzoom = true
		self.Zoomt = 0
		--this makes it so it ONLY does this bit of function instead of everything else too

		return
	end

	--just in case if you switched then it doesnt zoom in with the gun you switched to
	--makes you zoom back into the first scopelevel
	if self.Zoomt == 1 then
		self:GetOwner():SetFOV(self.Zoom1, 0.2)
		...
		self:GetOwner():SetFOV(self.Zoom2, 0.2)
		self.Zoom = 2
		self:SetNWInt("Zoom", 2)
		self.Zoomt = 0
...
--zooming out after pulling the bolt
...
--[[function SWEP:RCSIronsights()
	...
	if self:GetNWInt("Zoom") == 1 then return end
	if self:GetNWInt("Zoom") == 2 then return end
	if (!self.IronSightsPos) then return end
	if (self.NextSecondaryAttack > CurTime()) then return end
	//RealCS (including all this new spread and stuff, which is in v6/v7) is by ch/ee/sy/lar/d.
	bIronsights = !self:GetNetworkedBool("Ironsights", false)
