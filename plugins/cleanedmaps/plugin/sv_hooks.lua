...
function cwCleanedMaps:InitPostEntity()
	...
	timer.Simple(1, function()
		...
		if coreOne and IsValid(coreOne[1]) then
			coreOne[1]:Remove()
		end

		if coreTwo and IsValid(coreTwo[1]) then
			coreTwo[1]:Remove()
		end
	...
	if string.lower(game.GetMap()) == "rp_evocity_v2d" then
		--[[ local barricades = {
			{
				angles = Angle(-0.482, -8.456, -93.239),
				position = Vector(-6378.7979, -7698.8604, 224.9174)
			},
			{
				angles = Angle(-1.011, 1.972, -91.502),
				position = Vector(-6382.2466, -7669.2017, 252.3531)
			},
			{
				angles = Angle(0, 0, -90),
				position = Vector(-6407.5703, -7513.1294, 165.1916)
			}
		} ]]

		local entities = {ents.Create("prop_physics"), ents.Create("prop_physics"), ents.Create("prop_physics")}
		...
		for k, v in pairs(entities) do
			v:Spawn()
			Clockwork.entity:MakeSafe(v, true, true, true)
		end
	end
end
