...
function cwWeaponSelect:HUDPaintImportant()
	...
	local x = ScrW() / 6
	local y = ScrH() / 4
	if not IsValid(activeWeapon) or self.displayAlpha == 0 then return end
	...
	if not Clockwork.plugin:Call("PreDrawWeaponList", x, y, weaponLimit, self.displayAlpha, beforeWeapons, currentWeapon, afterWeapons, newWeapons) then
		if #beforeWeapons > 1 then
			for k, v in pairs(beforeWeapons) do
				local weaponAlpha = math.min((255 / weaponLimit) * k, self.displayAlpha)

				y = Clockwork.kernel:DrawInfo(string.upper(self:GetWeaponPrintName(v)), x, y, colorWhite, weaponAlpha, true, function(x, y, width, height)
		...
		if IsValid(currentWeapon) then
			...
			y = Clockwork.kernel:DrawInfo(currentWeaponName, x, y, informationColor, self.displayAlpha, true, function(x, y, width, height)
			...
			self:DrawWeaponInformation(Clockwork.item:GetByWeapon(currentWeapon), currentWeapon, weaponInfoX, weaponInfoY, self.displayAlpha)

			if #newWeapons == 1 then
				y = Clockwork.kernel:DrawInfo("There are no other weapons.", x, y, colorWhite, self.displayAlpha, true, function(x, y, width, height)
					Clockwork.plugin:Call("DrawWeaponList", x, y, width, height, self.displayAlpha, "current")
...
function cwWeaponSelect:PreDrawWeaponList(x, y, weaponLimit, alpha)
end

-- Called when the weapon list is drawn.
function cwWeaponSelect:DrawWeaponList(x, y, w, h, alpha, type)
end
...
function cwWeaponSelect:TopLevelPlayerBindPress(player, bind, bPress)
	...
	if #newWeapons == 1 and Clockwork.config:Get("weapon_selection_multi"):Get() then return end

	if string.find(bind, "invnext") or string.find(bind, "slot2") then
		if curTime >= self.displayDelay and not bPress then
			if #newWeapons > 1 then
