...
function cwWeaponSelect:DrawWeaponInformation(itemTable, weapon, x, y, alpha)
	...
	if not weapon.Primary or not weapon.Primary.ClipSize or weapon.Primary.ClipSize > 0 then
		if clipOne >= 0 then
			primaryAmmo = "Primary: " .. clipOne .. "/" .. clipOneAmount .. "."
		end
	end

	if not weapon.Secondary or not weapon.Secondary.ClipSize or weapon.Secondary.ClipSize > 0 then
		if clipTwo >= 0 then
	...
	if not weapon.Purpose then
		weapon.Purpose = ""
	end

	if not weapon.Contact then
		weapon.Contact = ""
	end

	if not weapon.Author then
		weapon.Author = ""
	end

	if itemTable or primaryAmmo or secondaryAmmo or weapon.DrawWeaponInfoBox and (weapon.Author ~= "" or weapon.Contact ~= "" or weapon.Purpose ~= "" or weapon.Instructions ~= "") then
