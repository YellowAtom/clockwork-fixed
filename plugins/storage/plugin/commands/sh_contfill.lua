		...
		if IsValid(trace.Entity) then
			if Clockwork.entity:IsPhysicsEntity(trace.Entity) then
				...
				if cwStorage.containerList[model] then
					if not trace.Entity.cwInventory then
					...
					if not arguments[2] or cwStorage:CategoryExists(arguments[2]) then
						while weight < containerWeight do
							local randomItem = cwStorage:GetRandomItem(arguments[2])

							if randomItem then
								Clockwork.inventory:AddInstance(trace.Entity.cwInventory, Clockwork.item:CreateInstance(randomItem[1]))
