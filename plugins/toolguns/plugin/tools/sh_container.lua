		...
		if IsValid(trace.Entity) then
			if Clockwork.entity:IsPhysicsEntity(trace.Entity) then
				...
				if cwStorage.containerList[model] then
					...
					if not category or cwStorage:CategoryExists(category) then
						while weight < containerWeight do
							local randomItem = cwStorage:GetRandomItem(category)

							if randomItem then
								Clockwork.inventory:AddInstance(trace.Entity.inventory, Clockwork.item:CreateInstance(randomItem[1]))
...
if plugin then
	if not Clockwork.plugin:IsDisabled(plugin.name) and not Clockwork.plugin:IsUnloaded(plugin.name) then
		TOOL:Register()
	end
end
