				...
				if (Clockwork.player:CanAfford(player, cost * amount)) then
					...
					if (player:CanHoldWeight(itemTable("weight") * amount)) then
						...
						if (amount > 1) then
							...
							Clockwork.kernel:PrintLog(LOGTYPE_MAJOR, player:Name().." has purchased "..amount.." "..Clockwork.kernel:Pluralize(itemTable("name")).." from "..data.entity:GetNetworkedString("Name").." leaving them with "..player:GetCash().." tokens."); -- Edited By TrulyDivined
						else
							...
							Clockwork.kernel:PrintLog(LOGTYPE_MAJOR, player:Name().." has purchased "..amount.." "..itemTable("name").." from "..data.entity:GetNetworkedString("Name").." leaving them with "..player:GetCash().." tokens."); -- Edited By TrulyDivined
						end;
				...
				if (itemTable) then
					...
					if (data.entity.cwCash == -1 or data.entity.cwCash >= cost) then
						if (player:TakeItem(itemTable)) then
							...
							Clockwork.kernel:PrintLog(LOGTYPE_MAJOR, player:Name().." has sold 1 "..itemTable("name").." to "..data.entity:GetNetworkedString("Name").." leaving them with "..player:GetCash().." tokens."); -- Edited By TrulyDivined remove if necessary		
						end;
					else
