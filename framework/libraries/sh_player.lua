...
--]]
	function Clockwork.player:CanHoldSpace(space)
		local inventorySpace = Clockwork.inventory:CalculateSpace(Clockwork.inventory:GetClient())
...
--]]
	function Clockwork.player:GetMaxWeight()
		local itemsList = Clockwork.inventory:GetAsItemsList(Clockwork.inventory:GetClient())
...
--]]
	function Clockwork.player:GetClothesItem()
		...
		if clothesData.itemID ~= nil and clothesData.uniqueID ~= nil then return Clockwork.inventory:FindItemByID(Clockwork.inventory:GetClient(), clothesData.uniqueID, clothesData.itemID) end
	end
...
--]]
	function Clockwork.player:HasAccessory(uniqueID)
		...
		for k, v in pairs(accessoryData) do
			if string.lower(v) == string.lower(uniqueID) then return true end
		end
...
--]]
	function Clockwork.player:IsNoClipping(player)
		if player:GetMoveType() == MOVETYPE_NOCLIP and not player:InVehicle() then return true end
	end
...
--]]
	function Clockwork.player:GetRealTrace(player, useFilterTrace)
		...
		if IsValid(newTrace.Entity) and not newTrace.HitWorld and (not IsValid(trace.Entity) or string.find(trace.Entity:GetClass(), "vehicle")) or useFilterTrace then
			trace = newTrace
		end
...
--]]
	function Clockwork.player:GetMaximumCharacters()
		...
		for k, v in pairs(Clockwork.faction.stored) do
			if not v.whitelist or table.HasValue(whitelisted, v.name) then
				maximum = maximum + 1
			end
		end
...
--]]
	function Clockwork.player:CanSeeNPC(player, target, allowance, ignoreEnts)
		if player:GetEyeTraceNoCursor().Entity == target then
			...
			if trace.Fraction >= (allowance or 0.75) then return true end
		end
	end
...
--]]
	function Clockwork.player:GetCharacterKey(player)
		if IsValid(player) then return player:GetSharedVar("Key") end
	end
...
--]]
	function Clockwork.player:GetPhysDesc(player)
		...
		if not physDesc or physDesc == "" then
...
--]]
	function Clockwork.player:GetDefaultSkin(player)
		local _, skin = Clockwork.class:GetAppropriateModel(player:Team(), player)
...
--]]
	function Clockwork.player:GetDefaultModel(player)
		local model, _ = Clockwork.class:GetAppropriateModel(player:Team(), player)
...
--]]
	function Clockwork.player:HasAnyFlags(player, flags, byDefault)
		...
		if playerFlags ~= "" then
			if Clockwork.class:HasAnyFlags(player:Team(), flags) and not byDefault then return true end
			...
			for i = 1, #flags do
				...
				if not byDefault then
					...
					if hasFlag ~= false then
						if hasFlag then return true end
					else
						wasSuccess = nil
					end
				end
...
--]]
	function Clockwork.player:HasFlags(player, flags, byDefault)
		...
		if playerFlags ~= "" then
			if Clockwork.class:HasFlags(player:Team(), flags) and not byDefault then return true end
			...
			for i = 1, #flags do
				...
				if not wasSuccess then
					...
					elseif flag == "o" then
						if not player:IsSuperAdmin() and not player:IsAdmin() then
							if not player:IsUserGroup("operator") then return end
						end
					elseif not string.find(playerFlags, flag) then
						return false
					end
				end
			end
	...
	function Clockwork.player:GetSharedVar(player, key, sharedTable)
		if IsValid(player) then
			if not sharedTable then
				...
				if sharedVarData.bPlayerOnly then
					if not sharedVarData.value then
					...
					local class = Clockwork.kernel:ConvertNetworkedClass(sharedVarData.class)
					if class then return player["GetNetworked" .. class](player, key) end
				end
			else
...
--]]
	function Clockwork.player:GetDrunk()
		...
		if isDrunk and isDrunk > 0 then return isDrunk end
	end
...
--]]
	function Clockwork.player:CreateCharacterFromData(player, data)
		if player.cwIsCreatingChar then return end
		...
		for k, v in pairs(cwClass:GetAll()) do
			if v.isOnCharScreen and (v.factions and table.HasValue(v.factions, factionTable.name)) then
				classes = true
			end
		end
		...
		if not factionTable.GetName then
			if not factionTable.useFullName then
			...
			elseif not data.fullName or data.fullName == "" then
				return self:SetCreateFault(player, {"FaultNameInvalid"})
			end
		end
		...
		if not cwFaction:IsGenderValid(info.faction, info.gender) then
			return self:SetCreateFault(player, {"FaultNeedGender"})
		end

		if factionTable.whitelist and not self:IsWhitelisted(player, info.faction) then
			return self:SetCreateFault(player, {"FaultNotOnWhitelist", info.faction})
		elseif cwFaction:IsModelValid(factionTable.name, info.gender, info.model) or factionTable.GetModel and not info.model then
			...
			if characterID then
				...
				elseif not factionTable.useFullName then
...
--]]
	function Clockwork.player:StartSound(player, uniqueID, sound, fVolume)
		if not player.cwSoundsPlaying then
			player.cwSoundsPlaying = {}
		end

		if not player.cwSoundsPlaying[uniqueID] or player.cwSoundsPlaying[uniqueID] ~= sound then
			player.cwSoundsPlaying[uniqueID] = sound

			cwDatastream:Start(player, "StartSound", {
				uniqueID = uniqueID,
				sound = sound,
				volume = fVolume or 0.75
			})
		end
	end
...
--]]
	function Clockwork.player:StripGear(player)
		if not player.cwGearTab then return end
...
--]]
	function Clockwork.player:GetWeaponRaised(player, bIsCached)
		if bIsCached then return player:GetSharedVar("IsWepRaised") end
		local weapon = player:GetActiveWeapon()

		if IsValid(weapon) and not weapon.NeverRaised then
			if weapon.GetRaised then
				local isRaised = weapon:GetRaised()
				if isRaised ~= nil then return isRaised end
			end
...
--]]
	function Clockwork.player:ToggleWeaponRaised(player)
		self:SetWeaponRaised(player, not player.cwWeaponRaiseClass)
	end
	...
	function Clockwork.player:SetWeaponRaised(player, isRaised)
		...
		if IsValid(weapon) then
			...
			elseif isRaised then
				if not player.cwWeaponRaiseClass then
	...
	function Clockwork.player:SetupRemovePropertyDelays(player, doAllCharacters)
		...
		for k, v in pairs(self:GetAllProperty()) do
			...
			if IsValid(v) and removeDelay then
				if uniqueID == cwEntity:QueryProperty(v, "uniqueID") and (doAllCharacters or key == cwEntity:QueryProperty(v, "key")) then
...
--]]
	function Clockwork.player:DisableProperty(player, characterOnly)
		...
		for k, v in pairs(self:GetAllProperty()) do
			if IsValid(v) and uniqueID == cwEntity:QueryProperty(v, "uniqueID") and (not characterOnly or key == cwEntity:QueryProperty(v, "key")) then
...
--]]
	function Clockwork.player:TakePropertyOffline(key, uniqueID, entity, anyCharacter)
		if key and uniqueID then
			...
			if cwEntity:QueryProperty(entity, "uniqueID") == uniqueID and cwEntity:QueryProperty(entity, "key") == key then
...
--]]
	function Clockwork.player:SetDrunk(player, expire)
		...
		elseif not player.cwDrunkTab then
		...
		if itemTable then
			...
			if primaryDefaultAmmo then
				...
				if type(primaryDefaultAmmo) == "number" then
					player:SetAmmo(math.max(player:GetAmmoCount(ammoClass) - primaryDefaultAmmo, 0), ammoClass)
				end
			end
			...
			if secondaryDefaultAmmo then
				...
				if type(secondaryDefaultAmmo) == "number" then
					player:SetAmmo(math.max(player:GetAmmoCount(ammoClass) - secondaryDefaultAmmo, 0), ammoClass)
				end
			end
		end
	end
...
--]]
	function Clockwork.player:EntityConditionTimer(player, target, entity, delay, distance, Condition, Callback)
		...
		cwKernel:CreateTimer("EntityCondTimer" .. uniqueID, 0, 0, function()
			...
			if IsValid(target) and IsValid(realEntity) and traceLine.Entity == realEntity and traceLine.Entity:GetPos():Distance(player:GetShootPos()) <= distance and Condition() then
				if CurTime() >= realDelay then
					Callback(true)
					player.cwConditionEntTimer = nil
					cwKernel:DestroyTimer("EntityCondTimer" .. uniqueID)
...
--]]
	function Clockwork.player:GiveFlags(player, flags)
		for i = 1, #flags do
			...
			if not string.find(player:GetFlags(), flag) then
...
--]]
	function Clockwork.player:GivePlayerFlags(player, flags)
		for i = 1, #flags do
			...
			if not string.find(player:GetPlayerFlags(), flag) then
...
--]]
	function Clockwork.player:GetMaximumCharacters(player)
		...
		for k, v in pairs(cwFaction:GetAll()) do
			if not v.whitelist or self:IsWhitelisted(player, v.name) then
				maximum = maximum + 1
			end
		end
...
--]]
	function Clockwork.player:Query(player, key, default)
		...
		if character then
			...
			if character[key] ~= nil then return character[key] end
		end
...
--		local closestPosition = nil
		...
		local angles = {math.NormalizeAngle(yawForward - 180), math.NormalizeAngle(yawForward - 135), math.NormalizeAngle(yawForward + 135), math.NormalizeAngle(yawForward + 45), math.NormalizeAngle(yawForward + 90), math.NormalizeAngle(yawForward - 45), math.NormalizeAngle(yawForward - 90), math.NormalizeAngle(yawForward)}
		...
		if not table.HasValue(filter, player) then
		...
		for k, v in pairs(directions) do
			...
			if not traceLine.Hit then break end
		end
		...
		for k, v in pairs(directions) do
			...
			local traceLine = util.TraceEntity(trace, player)
			if not traceLine.Hit then return traceLine.HitPos end
		end
...
--]]
	function Clockwork.player:HasAnyFlags(player, flags, byDefault)
		if player:GetCharacter() then
			...
			if cwClass:HasAnyFlags(player:Team(), flags) and not byDefault then return true end
...
--]]
	function Clockwork.player:HasFlags(player, flags, byDefault)
		if player:GetCharacter() then
			local playerFlags = player:GetFlags()
			if cwClass:HasFlags(player:Team(), flags) and not byDefault then return true end
...
--]]
	function Clockwork.player:GetDeathCode(player, authenticated)
		if player.cwDeathCodeIdx and (not authenticated or player.cwDeathCodeAuth) then return player.cwDeathCodeIdx end
	end
...
--]]
	function Clockwork.player:GetRealTrace(player, useFilterTrace)
		...
		if IsValid(newTrace.Entity) and (not IsValid(trace.Entity) or trace.Entity:IsVehicle()) and not newTrace.HitWorld or useFilterTrace then
			trace = newTrace
		end
...
--]]
	function Clockwork.player:ForceDeleteCharacter(player, characterID)
		...
		if character then
			...
			if not cwPlugin:Call("PlayerDeleteCharacter", player, character) then
...
--		local name = target:Name()
		...
		if status == RECOGNISE_SAVE then
			if cwCfg:Get("save_recognised_names"):Get() then
				if not cwPlugin:Call("PlayerCanSaveRecognisedName", player, target) then
		...
		if not status or shouldForce or not self:DoesRecognise(player, target, status) then
			...
			if not sharedTable then
				...
				if sharedVarData.bPlayerOnly then
					...
					if player.cwSharedVars[key] ~= realValue then
						...
						cwDatastream:Start(player, "SharedVar", {
							key = key,
							value = realValue
						})
					end
				...
				cwDatastream:Start(player, "SetSharedTableVar", {
...
--]]
	function Clockwork.player:GetSharedVar(player, key)
		if IsValid(player) then
			...
			if sharedVarData.bPlayerOnly then
				if not player.cwSharedVars[key] then
...
--]]
	function Clockwork.player:GetPropertyEntities(player, class)
		...
		for k, v in pairs(self:GetAllProperty()) do
			if uniqueID == cwEntity:QueryProperty(v, "uniqueID") then
				if key == cwEntity:QueryProperty(v, "key") then
					if not class or v:GetClass() == class then
...
--		local key = player:GetCharacterKey()
		...
		for k, v in pairs(self:GetAllProperty()) do
			if cwEntity:IsDoor(v) and not cwEntity:GetDoorParent(v) then
				if uniqueID == cwEntity:QueryProperty(v, "uniqueID") then
...
--]]
	function Clockwork.player:IsProtected(identifier)
		...
		if not wasSuccess or value == false then
		...
		if string.find(ownerSteamID, ",") then
			...
			for k, v in pairs(ownerSteamID) do
				if steamID and steamID == v then return true end
			end
		else
			if steamID and steamID == ownerSteamID then return true end
		end
	...
	function Clockwork.player:NotifyAdmins(adminLevel, text, icon)
		for k, v in pairs(player.GetAll()) do
			...
			elseif adminLevel == "admin" or adminLevel == "a" then
				if v:IsAdmin() and not v:IsUserGroup("operator") then
					self:Notify(v, text, true, icon)
				end
...
--]]
	function Clockwork.player:GetWeapons(player, shouldKeep)
		...
		for k, v in pairs(player:GetWeapons()) do
			...
			if not self:GetSpawnWeapon(player, class) then
				teamIndex = nil
			end
...
--]]
	function Clockwork.player:GetHolsteredWeapon(player)
		for k, v in pairs(player:GetWeapons()) do
			...
			if itemTable then
				if self:GetWeaponClass(player) ~= class then return class end
			end
		end
	end
...
--]]
	function Clockwork.player:PauseUnragdollTime(player)
		if not player.cwRagdollPaused then
...
--]]
	function Clockwork.player:GetRagdollEntity(player)
		if player.cwRagdollTab then
			if IsValid(player.cwRagdollTab.entity) then return player.cwRagdollTab.entity end
		end
	end
...
--]]
	function Clockwork.player:DoRagdollDecayCheck(player, ragdoll)
		...
		cwKernel:CreateTimer("DecayCheck" .. index, 60, 0, function()
			...
			if not playerIsValid and ragdollIsValid then
				if not cwEntity:IsDecaying(ragdoll) then
			...
			elseif not ragdollIsValid then
				cwKernel:DestroyTimer("DecayCheck" .. index)
			end
		end)
	end
...
--]]
	function Clockwork.player:LightSpawn(player, weapons, ammo, forceReturn)
		if player:IsRagdolled() and not forceReturn then
			self:SetRagdollState(player, RAGDOLL_NONE)
		end
	...
	function Clockwork.player:GetCharacterID(player)
		...
		if character then
			for k, v in pairs(player:GetCharacters()) do
				if v == character then return k end
			end
		end
	end
...
--]]
	function Clockwork.player:LoadCharacter(player, characterID, mergeCreate, Callback, shouldForce)
		...
		if mergeCreate then
			...
			if not player.cwCharacterList[characterID] then
...
--]]
	function Clockwork.player:LoadData(player, Callback)
		...
		queryObj:SetCallback(function(result)
			if not IsValid(player) or player.cwData then return end
			local onNextPlay = ""
			...
			if not player.cwUserGroup or player.cwUserGroup == "" then
				player.cwUserGroup = "user"
			end
		...
		timer.Simple(2, function()
			if IsValid(player) and not player.cwData then
...
--]]
	function Clockwork.player:CallThinkHook(player, setSharedVars, curTime)
		...
		if not player:IsJogging(true) then
...
function Clockwork.player:CanPromote(player, target)
	local _, rank = Clockwork.player:GetFactionRank(player)

	if rank then
		if rank.canPromote then
			local _, targetRank = Clockwork.player:GetFactionRank(target)
			local _, rankTable = Clockwork.faction:GetHighestRank(player:GetFaction())
			if targetRank.position and targetRank.position ~= rankTable.position then return rank.canPromote <= targetRank.position end
		end
	end
end
...
function Clockwork.player:CanDemote(player, target)
	...
	if rank then
		if rank.canDemote then
			local _, targetRank = Clockwork.player:GetFactionRank(target)
			local _, rankTable = Clockwork.faction:GetLowestRank(player:GetFaction())
			if targetRank.position and targetRank.position ~= rankTable.position then return rank.canDemote <= targetRank.position end
		end
	end
end
