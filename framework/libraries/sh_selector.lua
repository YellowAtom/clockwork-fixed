...
if SERVER then
	...
	Clockwork.datastream:Hook("Selector", function(player, data)
		...
		if type(page) == "number" and type(key) == "number" and type(text) == "string" and Clockwork.selector.active[player] then
	...
	function CLASS_TABLE:Select(key)
		...
		if tOption then
			if self.Callback then
				self.Callback(self.page, tOption.key, tOption.text)
			...
			if tOption.key ~= 7 and tOption.key ~= 8 then return true end
		end

		if not wasSuccess then return true end
	end
	...
	function CLASS_TABLE:Create()
		...
		if not self.paginated[self.page] then
			if self.pages[self.page - 1] then
				self.pages[self.page][#self.pages[self.page] + 1] = {
					class = "option",
					key = 7,
		...
		Clockwork.Client:AddPlayerOption("cwSelector", -1, function(key) return self:Select(key) end, function()
	...
	Clockwork.datastream:Hook("Selector", function(data)
		...
		if not Clockwork.Client or not Clockwork.Client:IsValid() then
