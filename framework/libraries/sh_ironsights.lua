...
function Clockwork.ironsights:ToggleIronSights(player)
	if SERVER then
		if IsValid(player) and player:IsValid() then
			...
			if cwConfig:Get("enable_ironsights"):Get() and cwPlugin:Call("PlayerCanToggleIronSights", player, showIronSights) then
				local curTime = CurTime()

				if not nextIronSights or nextIronSights <= curTime then
					self:SetIronSights(player, not showIronSights)
...
if SERVER then
	...
	function Clockwork.ironsights:PlayerCanSeeIronSights(player, showIronSights)
		if showIronSights then return not player:IsRunning() and cwPly:GetWeaponRaised(player) end
