...
function Clockwork.storage:SyncItem(player, itemTable)
	...
	if itemTable then
		...
		if player:HasItemInstance(itemTable) then
			Clockwork.datastream:Start(players, "StorageGive", {
...
function Clockwork.storage:GiveTo(player, itemTable)
	...
	if not storageTable.entity or not storageTable.entity:IsPlayer() then
		...
		if self:GetWeight(player) + math.max(weight, 0) > storageTable.weight or Clockwork.inventory:UseSpaceSystem() and self:GetSpace(player) + math.max(space, 0) > storageTable.space then return false end
	end
...
function Clockwork.storage:UpdateByID(player, uniqueID)
	...
	if itemTable and inventory[uniqueID] then
		...
		Clockwork.datastream:Start(player, "StorageGive", {
			index = itemTable("index"),
			itemList = itemList
		})
	end
