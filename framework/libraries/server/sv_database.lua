...
function Clockwork.database:Error(text)
end
...
function Clockwork.database:Query(query, Callback, flag, bRawQuery)
	...
	if self.MDB then
		...
		function sqlObject:onSuccess(data)
			if Callback and not bRawQuery then
