...
function Clockwork.directory:GetCategory(category)
	for k, v in pairs(self.stored) do
		if v.category == category then return v, k end
	end
end
...
function Clockwork.directory:AddCategoryMatch(category, findText, replaceText)
	if not self.matches[category] then
...
function Clockwork.directory:ReplaceMatches(category, htmlCode)
	htmlCode = string.gsub(htmlCode, "<lang>(.-)</lang>", function(target) return L(target) end)
	if not self.matches[category] then return htmlCode end
...
function Clockwork.directory:CategoryExists(category)
	for k, v in pairs(self.stored) do
		if v.category == category then return true end
	end
end
...
function Clockwork.directory:AddCategory(category, parent)
	...
	if not self:CategoryExists(category) then
...
function Clockwork.directory:RemoveCode(category, uniqueID, forceRemove)
	...
	if category then
		...
		if categoryTable then
			if uniqueID and not categoryTable.isHTML then
				...
				if not forceRemove and not categoryTable.isHTML then
					for k, v in pairs(self.stored) do
...
Clockwork.directory:SetCategorySorting("HelpCommands", function(a, b) return (a.sortData or a.htmlCode) < (b.sortData or b.htmlCode) end)
Clockwork.directory:SetCategorySorting("HelpPlugins", function(a, b) return (a.sortData or a.htmlCode) < (b.sortData or b.htmlCode) end)
