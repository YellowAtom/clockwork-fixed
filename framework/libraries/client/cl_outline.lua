...
function Clockwork.outline:Add(entity, glowColor, glowSize, bIgnoreZ)
	...
	halo.Add(entity, glowColor, glowSize, glowSize, 1, true, bIgnoreZ)
end
...
function Clockwork.outline:Fader(entity, glowColor, iDrawDist, bShowAnyway, tIgnoreEnts, glowSize, bIgnoreZ)
	...
	if not Clockwork.player:CanSeeEntity(Clockwork.Client, entity, 0.9, tIgnoreEnts) and not bShowAnyway then
		fOutlineAlpha = 0
	end

	if not entity.cwLastOutlineAlpha then
		entity.cwLastOutlineAlpha = 0
	end

	entity.cwLastOutlineAlpha = math.Approach(entity.cwLastOutlineAlpha, fOutlineAlpha, FrameTime() * 64)

	if entity.cwLastOutlineAlpha > 0 then
		self:Add(entity, Color(glowColor.r, glowColor.g, glowColor.b, entity.cwLastOutlineAlpha), glowSize, bIgnoreZ)
	end
end
