...
function Clockwork.system:GetActive()
	local panel = self:GetPanel()
	if panel then return panel.system end
end
...
function Clockwork.system:Register(system)
	...
	if not system.HasAccess then
		system.HasAccess = function(systemTable) return Clockwork.player:HasFlags(Clockwork.Client, systemTable.access) end
	end
