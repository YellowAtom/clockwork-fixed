...
if not chat.ClockworkAddText then
...
function Clockwork.chatBox:IsTypingCommand()
	...
	if string.find(currentText, prefix) == 1 then return true end
...
function Clockwork.chatBox:GetSpacing(fontName)
	...
	if textWidth and textHeight then return textHeight + 4 end
end
...
function Clockwork.chatBox:CreateDermaTextEntry()
	if not self.textEntry then
		...
		self.textEntry.OnKeyCodeTyped = function(textEntry, code)
			if code == KEY_ENTER and not textEntry:IsMultiline() and textEntry:GetEnterAllowed() then
				textEntry:FocusNext()
				textEntry:OnEnter()
				...
				if string.utf8sub(text, 1, string.utf8len(prefix)) == prefix then
					...
					if not exploded[2] then
						...
						for k, v in pairs(commands) do
							...
							if string.utf8len(command) < string.utf8len(v) and string.find(v, command) == 1 or useNextCmd then
		...
		self.scroll.OnMouseWheeled = function(panel, delta)
			...
			if isOpen then
				if delta > 0 then
					...
					if not self.historyMsgs[self.historyPos - delta] then
						delta = -1
					end
...
function Clockwork.chatBox:Decode(speaker, name, text, data, class, multiplier)
	...
	local filter = nil
	local icon = nil
	if not IsValid(Clockwork.Client) then return end
	...
	if IsValid(speaker) then
		if not Clockwork.kernel:IsChoosingCharacter() then
			if speaker:Name() ~= "" then
				...
				if not Clockwork.player:DoesRecognise(speaker, RECOGNISE_TOTAL) and filter == "ic" then
					unrecognised = true
				end
				...
				if info.visible then
					if info.filter == "ic" then
						if not Clockwork.Client:Alive() then return end
					end
...
function Clockwork.chatBox:Paint()
	...
	if not self.spaceWidths[chatBoxTextFont] then
	...
	for k, v in pairs(messages) do
		...
		if not self.spaceWidths[fontName] then
...
--			local newLine = false
			...
			if mouseX > messageX and mouseY > messageY and mouseX < messageX + v2.width and mouseY < messageY + v2.height then
				if v2.OnHover then
					onHoverData = v2
				end
			end
	...
	if isTypingCommand then
		...
		if command and command ~= "" then
			for k, v in pairs(Clockwork.command:GetAlias()) do
				...
				if string.utf8sub(k, 1, commandLen) == string.lower(command) and (not splitTable[2] or string.lower(command) == k) then
...
function Clockwork.chatBox:Add(filtered, icon, ...)
	if ScrW() == 160 or ScrH() == 27 then return end
		...
		if text then
			...
			for k, v in pairs(text) do
				if type(v) == "string" or type(v) == "number" or type(v) == "boolean" then
					Clockwork.chatBox:WrappedText(nil, message, curColor or colorWhite, tostring(v), curOnHover)
					curColor = nil
					curOnHover = nil
				elseif type(v) == "function" then
					curOnHover = v
				elseif type(v) == "Player" then
					Clockwork.chatBox:WrappedText(nil, message, cwTeam.GetColor(v:Team()), v:Name(), curOnHover)
					curColor = nil
					curOnHover = nil
				elseif type(v) == "table" then
...
Clockwork.chatBox:RegisterDefaultClass("looc", "ooc", function(info)
	if not Clockwork.config:Get("enable_looc_icons"):Get() then
		info.icon = nil
	end
...
Clockwork.datastream:Hook("ChatBoxDeathCode", function(data)
	...
	if Clockwork.chatBox:IsOpen() then
		...
		if text ~= "" and string.utf8sub(text, 1, 2) ~= "//" and string.utf8sub(text, 1, 3) ~= ".//" and string.utf8sub(text, 1, 2) ~= "[[" then
