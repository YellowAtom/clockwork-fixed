...
function Clockwork.inventory:AddInstance(inventory, itemTable, quantity)
	...
	if not inventory[itemTable("uniqueID")] then
...
function Clockwork.inventory:FindItemByID(inventory, uniqueID, itemID)
	...
	if not itemTable or not inventory[itemTable("uniqueID")] then return end
...
function Clockwork.inventory:IsEmpty(inventory)
	...
	for k, v in pairs(inventory) do
		if table.Count(v) > 0 then return false end
	end
...
function Clockwork.inventory:ToLoadable(inventory)
	...
	for k, v in pairs(inventory) do
		...
		if itemTable then
			local uniqueID = itemTable("uniqueID")
			if uniqueID ~= k then continue end

			if not newTable[uniqueID] then
				newTable[uniqueID] = {}
			end

			for k2, v2 in pairs(v) do
				local itemID = tonumber(k2)
				local instance = Clockwork.item:CreateInstance(k, itemID, v2)

				if instance and not instance.OnLoaded or instance:OnLoaded() ~= false then
...
function Clockwork.inventory:ToSaveable(inventory)
	...
	for k, v in pairs(inventory) do
		...
		for k2, v2 in pairs(v) do
			if type(v2) == "table" and (v2.IsInstance and v2:IsInstance()) then
				local newData = {}
				...
				if not v2.OnSaved or v2:OnSaved(newData) ~= false then
...
if CLIENT then
	...
	function Clockwork.inventory:HasEquipped(itemTable)
		if itemTable.HasPlayerEquipped then return itemTable:HasPlayerEquipped(Clockwork.Client) == true end
	...
	Clockwork.datastream:Hook("InvNetwork", function(data)
		...
		if itemTable then
			...
			if bHasEquipped ~= Clockwork.inventory:HasEquipped(itemTable) then
				Clockwork.inventory:Rebuild(Clockwork.menu:GetOpen())
			end
		end
	end)
	...
	Clockwork.datastream:Hook("InvTake", function(data)
		local itemTable = Clockwork.inventory:FindItemByID(Clockwork.inventory.client, data[1], data[2])
