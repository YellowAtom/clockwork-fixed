...
if SERVER then
	...
	function Clockwork.dermaRequest:Validate(player, data)
		if data.id and data.recv and self.hooks[data.id] and self.hooks[data.id].player == player then return true end

		return false
	end

	Clockwork.datastream:Hook("dermaRequestCallback", function(player, data)
		if not Clockwork.dermaRequest:Validate(player, data) then return end
	...
	function Clockwork.dermaRequest:Send(id, recv)
		Clockwork.datastream:Start("dermaRequestCallback", {
			id = id,
			recv = recv
		})
	end
	...
	Clockwork.datastream:Hook("dermaRequest_confirmQuery", function(data)
		Derma_Query(T(data.question), T(data.title), "Confirm", function()
			Clockwork.dermaRequest:Send(data.id, true)
		end, "Cancel", function()
			Clockwork.dermaRequest:Send(data.id, false)
		end)
	end)
