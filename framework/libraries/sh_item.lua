...
function CLASS_TABLE:__call(varName, failSafe)
	if self.queryProxies[varName] then
		...
		if type(dataName) ~= "function" then
			...
			if defaultValue ~= nil and currentValue ~= nil and (defaultValue ~= currentValue or not bNotDefault) then return self.data[dataName] end
		else
			local returnValue = dataName(self)
			if returnValue ~= nil then return returnValue end
		end
	end
	...
	--]]
	if self.data[varName] ~= nil then return self.data[varName] end
...
function CLASS_TABLE:IsBasedFrom(uniqueID)
	...
	if itemTable("unique") == uniqueID then return true end

	while itemTable and itemTable("baseItem") do
		if itemTable("baseItem") == uniqueID then return true end
		itemTable = Clockwork.item:FindByID(itemTable("baseItem"))
	end
...
function CLASS_TABLE:CanBeOrdered()
	return not self("isBaseItem") and self("business")
end
...
function CLASS_TABLE:IsTheSameAs(itemTable)
	if itemTable then
		return itemTable("uniqueID") == self("uniqueID") and itemTable("itemID") == self("itemID")
	else
		return false
	end
end
	...
	function CLASS_TABLE:DeductFunds(player)
		if #self.recipes > 0 then
			for k, v in pairs(self.recipes) do
				if not Clockwork.kernel:HasObjectAccess(player, v) then continue end
		...
		if self("cost") == 0 then return end

		Clockwork.player:GiveCash(player, -(self("cost") * self("batch")), {
			"AmountOfThing", self("batch"), {self("name")}
		})

		Clockwork.kernel:PrintLog(LOGTYPE_MINOR, {
			"LogPlayerOrdered", player:Name(), self("batch"), {self("name")}
		})
	end

	-- A function to get whether a player can afford to order the item.
	function CLASS_TABLE:CanPlayerAfford(player)
		if not Clockwork.player:CanAfford(player, self("cost") * self("batch")) then return false end
		...
		if #self.recipes > 0 then
			for k, v in pairs(self.recipes) do
				...
				if hasIngredients then return true end
			end
...
if SERVER then
	...
	function CLASS_TABLE:NetworkData()
		...
		if Clockwork.kernel:TimerExists(timerName) then return end

		Clockwork.kernel:CreateTimer(timerName, 1, 1, function()
			Clockwork.item:SendUpdate(self, self.networkQueue)
			self.networkQueue = {}
		end)
	end
else
...
function Clockwork.item:CreateCopy(itemTable)
	return Clockwork.item:CreateInstance(itemTable("uniqueID"), nil, itemTable("data"))
end

-- A function to get whether an item is a weapon.
function Clockwork.item:IsWeapon(itemTable)
	if itemTable and itemTable:IsBasedFrom("weapon_base") then return true end
...
function Clockwork.item:GetByWeapon(weapon)
	if IsValid(weapon) then
		...
		if itemID and itemID ~= 0 then return self:FindInstance(itemID) end
	end
end
...
function Clockwork.item:CreateInstance(uniqueID, itemID, data)
	...
	if itemTable then
		...
		if not self.instances[itemID] then
...
function Clockwork.item:FindByID(identifier)
	if identifier and identifier ~= 0 and type(identifier) ~= "boolean" then
		...
		for k, v in pairs(self.stored) do
			...
			if string.find(string.lower(itemName), lowerName) and (not itemTable or string.utf8len(itemName) < string.utf8len(itemTable("name"))) then
				itemTable = v
			end
		end
...
function Clockwork.item:Merge(itemTable, baseItem, bTemporary)
	...
	if baseTable and baseTable ~= itemTable then
		...
		if baseTableCopy.baseItem then
			baseTableCopy = self:Merge(baseTableCopy, baseTableCopy.baseItem, true)
			if not baseTableCopy then return end
		end
...
function Clockwork.item:Initialize()
	...
	for k, v in pairs(itemsTable) do
		if v.baseItem and not self:Merge(v, v.baseItem) then
			itemsTable[k] = nil
		end
	end
...
if SERVER then
		...
		if player:HasItemInstance(itemTable) then
			if itemTable.OnUse then
				if itemEntity and itemTable.noPlacementUse then
					return false
				elseif not itemEntity and itemTable.noInventoryUse then
					return false
				end
	...
	function Clockwork.item:Drop(player, itemTable, position, noSound, dontTake)
		if itemTable and (dontTake or player:HasItemInstance(itemTable)) then
			...
			if itemTable.OnDrop then
				...
				if itemTable:OnDrop(player, position) == false then return false end

				if not dontTake then
					player:TakeItem(itemTable)
				end
	...
	function Clockwork.item:Destroy(player, itemTable, noSound)
		if player:HasItemInstance(itemTable) and itemTable.OnDestroy then
			if itemTable:OnDestroy(player) == false then return false end
			player:TakeItem(itemTable)
	...
	function Clockwork.item:SendToPlayer(player, itemTable)
		if itemTable then
			Clockwork.datastream:Start(player, "ItemData", Clockwork.item:GetDefinition(itemTable, true))
		end
	end
	...
	function Clockwork.item:SendUpdate(itemTable, data)
		...
		if Clockwork.plugin:Call("ItemGetNetworkObservers", itemTable, info) or info.sendToAll then
			info.observers = nil
		end
	...
	function Clockwork.item:GetMarkupToolTip(itemTable, isBusinessStyle, Callback)
		...
		if itemTable.GetClientSideName and itemTable:GetClientSideName() then
		...
		if itemTable.GetClientSideInfo and itemTable:GetClientSideInfo() then
			toolTip = itemTable:GetClientSideInfo(markupObject)
		end

		if itemTable.GetClientSideDescription and itemTable:GetClientSideDescription() then
		...
		if isBusinessStyle then
					...
					if Clockwork.kernel:HasObjectAccess(Clockwork.Client, v) then
						...
						for k2, v2 in pairs(v.ingredients) do
							...
							if requiredItem then
								local numItems = Clockwork.inventory:GetItemsByID(Clockwork.inventory:GetClient(), k2)
			...
			if Clockwork.config:Get("cash_enabled"):Get() and totalCost ~= 0 then
	...
	Clockwork.datastream:Hook("ItemData", function(data)
		Clockwork.item:CreateInstance(data.uniqueID or data.index, data.itemID, data.data)
	end)
end
