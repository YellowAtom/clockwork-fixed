...
function PANEL:Init()
	...
	if Clockwork.door:GetOwner() == Clockwork.Client then
		...
		if not Clockwork.door:IsUnsellable() then
...
function PANEL:Think()
	...
	if not IsValid(entity) or entity:GetPos():Distance(Clockwork.Client:GetPos()) > 192 then
		self:Close()
		self:Remove()
		gui.EnableScreenClicker(false)
	end
...
Clockwork.datastream:Hook("DoorManagement", function(data)
	...
	if not Clockwork.entity:HasOwner(data.entity) or IsValid(data.owner) then
