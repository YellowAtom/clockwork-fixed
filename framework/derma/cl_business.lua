...
function PANEL:Init()
	...
	if customData.cooldown then
		self.spawnIcon:SetCooldown(customData.cooldown.expireTime, customData.cooldown.textureID)
	end
...
function PANEL:Init()
	...
	if not self.itemTable.originalCost then
...
-- Called each frame.
function PANEL:Think()
	if not self.nextUpdateMarkup then
		self.nextUpdateMarkup = 0
	end

	if CurTime() < self.nextUpdateMarkup then return end
