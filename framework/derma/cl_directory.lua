...
function PANEL:ShowCategory(category)
	if not category then
		...
		if categoryTable then
			if not categoryTable.isHTML then
				local newPageData = {}
					...
					for k, v in pairs(newPageData) do
						...
						if htmlCode and htmlCode ~= "" then
							if not firstKey then
								if (not formatting or not formatting.noLineBreaks) and not v.noLineBreak then
			...
			elseif not categoryTable.isWebsite then
				local masterFormatting = Clockwork.directory:GetMasterFormatting()
				local formatting = Clockwork.directory:GetCategoryFormatting(category)
...
function PANEL:Rebuild()
	if not CW_REBUILDING_DIRECTORY then
		self:ClearNodes()
		...
		table.sort(Clockwork.directory.stored, function(a, b) return a.category < b.category end)
		local nodeTable = {}
		...
		for k, v in pairs(Clockwork.directory.stored) do
			...
			elseif not nodeTable[v.category] then
				nodeTable[v.category] = self.treeNode:AddNode(L(v.category))
			end

			if not nodeTable[v.category].Initialized then
				...
				nodeTable[v.category].DoClick = function(node)
					for k2, v2 in pairs(Clockwork.directory.stored) do
						if v2.category == v.category and (v2.isWebsite or v2.isHTML or #v2.pageData > 0) then
