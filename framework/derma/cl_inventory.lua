...
function PANEL:HandleUnequip(itemTable)
	if itemTable.OnHandleUnequip then
		itemTable:OnHandleUnequip(function(arguments)
			if arguments then
...
function PANEL:Rebuild()
	...
	table.sort(categories.equipment, function(a, b) return a.category < b.category end)
	...
	table.sort(categories.inventory, function(a, b) return a.category < b.category end)
	...
	if #categories.equipment > 0 then
		for k, v in pairs(categories.equipment) do
			...
			for k2, v2 in pairs(v.itemsList) do
				...
				categoryList:AddItem(vgui.Create("cwInventoryItem", self))
			end
...
function PANEL:Think()
	for k, v in pairs(Clockwork.Client:GetWeapons()) do
		...
		if weaponItem and not v.cwIsWeaponItem then
...
-- Called each frame.
function PANEL:Think()
	if not self.nextUpdateContents then
		self.nextUpdateContents = CurTime() + 0.1
	end

	if CurTime() < self.nextUpdateContents then return end
