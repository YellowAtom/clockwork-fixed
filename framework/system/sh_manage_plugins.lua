...
if CLIENT then
	...
	function SYSTEM:OnDisplay(systemPanel, systemForm)
		...
		table.sort(mainPlugins, function(a, b) return a.category < b.category end)
			...
			for k, v in pairs(mainPlugins) do
				...
				for k2, v2 in pairs(v.plugins) do
					...
					self.pluginButtons[v2.name].DoClick = function(button)
						if not Clockwork.plugin:IsDisabled(v2.name) then
	...
	Clockwork.datastream:Hook("SystemPluginSet", function(player, data)
		...
		if data[2] == true and (not loadTable or not Clockwork.player:HasFlags(player, loadTable.access)) then
			return
		elseif data[2] == false and (not unloadTable or not Clockwork.player:HasFlags(player, unloadTable.access)) then
			return
