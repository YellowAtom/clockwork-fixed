...
if CLIENT then
	...
	function SYSTEM:OnDisplay(systemPanel, systemForm)
		if self.groupType == GROUP_USER then
			...
			local userGroups = {L("ManageGroupsSuperAdmins"), L("ManageGroupsAdmins"), L("ManageGroupsOperators")}
	...
	Clockwork.datastream:Hook("SystemGroupDemote", function(player, data)
		...
		if commandTable and type(data) == "table" and Clockwork.player:HasFlags(player, commandTable.access) then
	...
	Clockwork.datastream:Hook("SystemGroupGet", function(player, data)
		...
		if groupPage then
			...
			queryObj:SetCallback(function(result)
				...
				table.sort(groupPlayers, function(a, b) return a.steamName < b.steamName end)
