...
function COMMAND:OnRun(player, arguments)
	...
	if not itemTable or not itemTable:CanBeOrdered() then return false end
	...
	if not Clockwork.kernel:HasObjectAccess(player, itemTable) then
		Clockwork.player:Notify(player, {"NoAccessToOrderItem"})

		return false
	end

	if not Clockwork.plugin:Call("PlayerCanOrderShipment", player, itemTable) then return false end
	...
	if itemTable:CanPlayerAfford(player) then
		...
		if player:GetShootPos():Distance(trace.HitPos) <= 192 then
			if itemTable.CanOrder and itemTable:CanOrder(player, v) == false then return false end
			...
			if itemTable("batch") > 1 and entity.cwInventory then
				local itemTables = Clockwork.inventory:GetItemsByID(entity.cwInventory, itemTable("uniqueID"))
