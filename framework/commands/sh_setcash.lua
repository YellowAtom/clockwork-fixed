	...
	if target then
		if cash and cash >= 1 then
			...
			Clockwork.player:Notify(player, {
				"YouSetPlayersCash", targetName, {"Cash"},
				Clockwork.kernel:FormatCash(cash, nil, true)
			})

			Clockwork.player:Notify(target, {
				"YourCashSetBy", {"Cash"},
				Clockwork.kernel:FormatCash(cash, nil, true), playerName
			})
		else
