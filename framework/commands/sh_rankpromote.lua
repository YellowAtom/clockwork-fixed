...
function COMMAND:OnRun(player, arguments)
	...
	if isForced then
		if player:IsAdmin() then
			local _, rankTable = Clockwork.faction:GetHighestRank(target:GetFaction())
			local _, targetRankTable = Clockwork.player:GetFactionRank(target)

			if istable(rankTable) and targetRankTable.position and targetRankTable.position ~= rankTable.position then
				local _, rankTable = target:GetFactionRank()
