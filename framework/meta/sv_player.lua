...
function playerMeta:RemoveAccessory(itemTable)
	...
	Clockwork.datastream:Start(self, "RemoveAccessory", {
		itemID = itemID
	})
...
function playerMeta:WearAccessory(itemTable)
	...
	Clockwork.datastream:Start(self, "AddAccessory", {
		itemID = itemID,
		uniqueID = uniqueID
	})
...
function playerMeta:SetFakingDeath(fakingDeath, killSilent)
	...
	if not fakingDeath and killSilent then
		self:KillSilent()
	end
end
...
function playerMeta:HasTrait(uniqueID)
	...
	if traits and table.HasValue(traits, uniqueID) then return true end
end

-- A function to give a weapon to a player.
function playerMeta:Give(class, itemTable, bForceReturn)
	local iTeamIndex = self:Team()
	if not cwPlugin:Call("PlayerCanBeGivenWeapon", self, class, itemTable) then return end

	if self:IsRagdolled() and not bForceReturn then
		...
		if not spawnWeapon then
			iTeamIndex = nil
		end

		for k, v in pairs(ragdollWeapons) do
			if v.weaponData["class"] == class and v.weaponData["itemTable"] == itemTable then
	...
	elseif not self:HasWeapon(class) then
		...
		if IsValid(weapon) and itemTable then
			...
			weapon:SetNWString("ItemID", tostring(itemTable("itemID")))
...
function playerMeta:IsRunning(bNoWalkSpeed)
	if self:Alive() and not self:IsRagdolled() and not self:InVehicle() and not self:Crouching() and self:KeyDown(IN_SPEED) then
...
function playerMeta:StripWeapons(ragdollForce)
	if self:IsRagdolled() and not ragdollForce then
...
function playerMeta:GetAttributes()
	return self:QueryCharacter("Attributes")
end

-- A function to get a player's saved ammo.
function playerMeta:GetSavedAmmo()
	return self:QueryCharacter("Ammo")
end

-- A function to get a player's default model.
function playerMeta:GetDefaultModel()
	return self:QueryCharacter("Model")
end

-- A function to get a player's character ID.
function playerMeta:GetCharacterID()
	return self:QueryCharacter("CharacterID")
end

-- A function to get the time when a player's character was created.
function playerMeta:GetTimeCreated()
	return self:QueryCharacter("TimeCreated")
end

-- A function to get a player's character key.
function playerMeta:GetCharacterKey()
	return self:QueryCharacter("Key")
end
...
function playerMeta:GetCharacter()
	return cwPly:GetCharacter(self)
end

-- A function to get a player's storage table.
function playerMeta:GetStorageTable()
	return cwStorage:GetTable(self)
end

-- A function to get a player's ragdoll table.
function playerMeta:GetRagdollTable()
	return cwPly:GetRagdollTable(self)
end
...
function playerMeta:GetStorageEntity()
	return cwStorage:GetEntity(self)
end
...
function playerMeta:SetForcedAnimation(animation, delay, OnAnimate, OnFinish)
	...
	local bIsPermanent = not delay or delay == 0
	local bShouldPlay = not forcedAnimation or forcedAnimation.delay ~= 0
	...
	if bShouldPlay then
		...
		if bIsPermanent then
			cwKernel:DestroyTimer("ForcedAnim" .. self:UniqueID())
		else
		...
		Clockwork.datastream:Start(nil, "ForcedAnimResetCycle", {
			player = self
		})
...
function playerMeta:GetItemEntity()
	if IsValid(self.itemEntity) then return self.itemEntity end
end
...
function playerMeta:CreateTempData()
	...
	if not Clockwork.TempPlayerData[uniqueID] then
...
function playerMeta:GetItemsByID(uniqueID)
	return cwInventory:GetItemsByID(self:GetInventory(), uniqueID)
end

-- A function to find a player's items by name.
function playerMeta:FindItemsByName(uniqueID, name)
	return cwInventory:FindItemsByName(self:GetInventory(), uniqueID, name)
end
...
function playerMeta:CanHoldWeight(weight)
	local inventoryWeight = cwInventory:CalculateWeight(self:GetInventory())
...
function playerMeta:CanHoldSpace(space)
	if not cwInventory:UseSpaceSystem() then return true end
	local inventorySpace = cwInventory:CalculateSpace(self:GetInventory())
...
function playerMeta:HasItemByID(uniqueID)
	return cwInventory:HasItemByID(self:GetInventory(), uniqueID)
end

-- A function to count how many items a player has by ID.
function playerMeta:GetItemCountByID(uniqueID)
	return cwInventory:GetItemCountByID(self:GetInventory(), uniqueID)
end
...
function playerMeta:HasItemCountByID(uniqueID, amount)
	return cwInventory:HasItemCountByID(self:GetInventory(), uniqueID, amount)
end

-- A function to find a player's item by ID.
function playerMeta:FindItemByID(uniqueID, itemID)
	return cwInventory:FindItemByID(self:GetInventory(), uniqueID, itemID)
end
...
function playerMeta:HasItemAsWeapon(itemTable)
	for k, v in pairs(self:GetWeapons()) do
		...
		if itemTable:IsTheSameAs(weaponItemTable) then return true end
	end
...
function playerMeta:FindWeaponItemByID(uniqueID, itemID)
	for k, v in pairs(self:GetWeapons()) do
		local weaponItemTable = cwItem:GetByWeapon(v)
		if weaponItemTable and weaponItemTable("uniqueID") == uniqueID and weaponItemTable("itemID") == itemID then return weaponItemTable end
	end
end

-- A function to get whether a player has an item instance.
function playerMeta:HasItemInstance(itemTable)
	return cwInventory:HasItemInstance(self:GetInventory(), itemTable)
end
...
function playerMeta:GiveItem(itemTable, isForced)
	...
	if not itemTable or not itemTable:IsInstance() then
		debug.Trace()
	...
	if self:CanHoldWeight(itemTable("weight")) and self:CanHoldSpace(itemTable("space")) or isForced then
		if itemTable.OnGiveToPlayer then
			itemTable:OnGiveToPlayer(self)
		end

		cwKernel:PrintLog(LOGTYPE_GENERIC, {
			"LogPlayerGainedItem", self:Name(), {itemTable("name")},
			itemTable("itemID")
		})
...
function playerMeta:TakeItem(itemTable)
	...
	cwKernel:PrintLog(LOGTYPE_GENERIC, {
		"LogPlayerLostItem", self:Name(), {itemTable("name")},
		itemTable("itemID")
	})
...
function playerMeta:GetClothesItem()
	...
	if type(clothesData) == "table" then
		if clothesData.itemID ~= nil and clothesData.uniqueID ~= nil then return self:FindItemByID(clothesData.uniqueID, clothesData.itemID) end
	end
end
