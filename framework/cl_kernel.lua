...
function Clockwork:ClockworkConVarChanged(name, previousValue, newValue)
	if checkTable[name] and not cwTheme:IsFixed() then
...
function Clockwork:CalcView(player, origin, angles, fov)
	...
	elseif not cwClient:Alive() then
		return {
			origin = Vector(20000, 0, 0),
			angles = Angle(0, 0, 0),
			fov = fov
	...
	elseif cwConfig:Get("enable_headbob"):Get() and scale > 0 then
		if player:IsOnGround() then
			...
			if not cwPly:IsNoClipping(player) then
...
function Clockwork:CalcViewModelView(weapon, viewModel, oldEyePos, oldEyeAngles, eyePos, eyeAngles)
	...
	if not Clockwork.Client:HasInitialized() or not Clockwork.config:HasInitialized() or Clockwork.Client:GetMoveType() == MOVETYPE_OBSERVER then
		weaponRaised = nil
	end
...
function Clockwork:HUDShouldDraw(name)
	if not IsValid(cwClient) or not cwClient:HasInitialized() or cwKernel:IsChoosingCharacter() then
...
function Clockwork:Tick()
	...
	if cwCharacter:IsPanelPolling() then
		...
		if not panel and cwPlugin:Call("ShouldCharacterMenuBeCreated") then
	...
	if IsValid(cwClient) and not cwKernel:IsChoosingCharacter() then
		self.bars.stored = {}
		...
		table.sort(self.PlayerInfoText.subText, function(a, b) return a.priority > b.priority end)
		...
		if cwConfig:Get("fade_dead_npcs"):Get() then
			for k, v in pairs(ents.FindByClass("class C_ClientRagdoll")) do
				if not cwEntity:IsDecaying(v) then
		...
		if cwClient:Alive() and cwConfig:Get("enable_heartbeat"):Get() then
			...
			if health < 25 then
				...
				if not self.NextHeartbeat or curTime >= self.NextHeartbeat then
		...
		if not playedHeartbeatSound and self.HeartbeatSound then
			self.HeartbeatSound:Stop()
		end
	end

	if not self.NextHandleAttributeBoosts or realCurTime >= self.NextHandleAttributeBoosts then
	...
	if cwKernel:IsInfoMenuOpen() and not input.IsKeyDown(KEY_F1) then
		cwKernel:RemoveBackgroundBlur("InfoMenu")
		cwKernel:CloseActiveDermaMenus()
		Clockwork.InfoMenuOpen = false
	...
	if menuMusic ~= "" then
		...
		elseif self.MusicSound and not self.MusicFading then
...
function Clockwork:HUDPaintForeground()
	...
	if info then
		...
		cwKernel:DrawBar(x, y, width, height, info.color or cwOption:GetColor("information"), info.text or "Progress Bar", info.percentage or 100, 100, info.flash, {
	...
	local info = {
		width = ScrW() * cwOption:GetKey("top_bar_width_scale"),
		x = 16,
		y = 16
	}
...
function Clockwork:GetProgressBarInfo()
	...
	if not cwClient:Alive() and action == "spawn" then
		return {
			text = L("YouWillRespawnSoon"),
			percentage = percentage,
			flash = percentage < 10
		}
	end

	if not cwClient:IsRagdolled() then
		if action == "lock" then
			return {
				text = L("EntityBeingLocked"),
				percentage = percentage,
				flash = percentage < 10
			}
		elseif action == "unlock" then
			return {
				text = L("EntityBeingUnlocked"),
				percentage = percentage,
				flash = percentage < 10
			}
		end
	elseif action == "unragdoll" then
		if cwClient:GetRagdollState() == RAGDOLL_FALLENOVER then
			return {
				text = L("YouAreGainingStability"),
				percentage = percentage,
				flash = percentage < 10
			}
		else
			return {
				text = L("YouAreGainingConciousness"),
				percentage = percentage,
				flash = percentage < 10
			}
		end
...
function Clockwork:PlayerAdjustHeadbobInfo(info)
	...
	if bIsDrunk then
		info.speed = info.speed * math.min(bIsDrunk * 0.25, 4)
...
function Clockwork:HUDDrawScoreBoard()
	...
	if not cwPly:HasDataStreamed() then
		if not self.DataStreamedAlpha then
	...
	if self.CharacterLoadingFinishTime then
		...
		if not self.CinematicBarsDrawn then
			cwKernel:DrawCinematicIntroBars()
		end
	end
...
concommand.Add("cwSay", function(player, command, arguments) return cwDatastream:Start("PlayerSay", table.concat(arguments, " ")) end)
