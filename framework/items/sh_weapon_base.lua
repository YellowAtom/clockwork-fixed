...
function ITEM:OnPlayerUnequipped(player, extraData)
	...
	if (itemTable:IsTheSameAs(self)) then
		...
		elseif (hook.Run("PlayerCanDropWeapon", player, self, weapon)) then
			...
			if (player:GetShootPos():DistToSqr(trace.HitPos) <= 36864) then
...
-- A function to get whether the item is a primary weapon.
function ITEM:IsPrimaryWeapon()
	return self("isPrimaryWeapon");
end;

-- A function to get whether the item is a secondary weapon.
