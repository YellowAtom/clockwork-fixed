...
function ITEM:GetModelName(player, group)
	...
	if not string.find(modelName, "male") and not string.find(modelName, "female") then
		if group then
			group = "group05/"
		else
			group = ""
...
function ITEM:OnUse(player, itemEntity)
	if self("whitelist") and not table.HasValue(self("whitelist"), player:GetFaction()) then
	...
	if player:Alive() and not player:IsRagdolled() then
		if not self.CanPlayerWear or self:CanPlayerWear(player, itemEntity) ~= false then
