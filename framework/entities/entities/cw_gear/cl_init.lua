...
-- Called each frame.
function ENT:Think()
	if not Clockwork.entity:HasFetchedItemData(self) then
	...
	if IsValid(player) then
		...
		if (eyePos:Distance(playerEyePos) > 32 or GetViewEntity() ~= Clockwork.Client or Clockwork.Client ~= player or not isPlayer) and (not isPlayer or player:Alive()) then
...
function ENT:Draw()
	if not Clockwork.entity:HasFetchedItemData(self) then return end
	local playerEyePos = Clockwork.Client:EyePos()
	...
	if IsValid(player) and (player:GetMoveType() == MOVETYPE_WALK or player:IsRagdolled() or player:InVehicle()) then
		...
		if (eyePos:Distance(playerEyePos) > 32 or GetViewEntity() ~= Clockwork.Client or Clockwork.Client ~= player or not isPlayer) and (not isPlayer or player:Alive()) and colorTable.a > 0 then
			bDrawModel = true
		end
	end
