...
function Clockwork.kernel:URLEncode(url)
	...
	for i = 1, #url do
		...
		if a < 128 then
			if a == 32 or a >= 34 and a <= 38 or a == 43 or a == 44 or a == 47 or a >= 58 and a <= 64 or a >= 91 and a <= 94 or a == 96 or a >= 123 and a <= 126 then
...
function Clockwork.kernel:AreTablesEqual(tableA, tableB)
	if istable(tableA) and istable(tableB) then
		for k, v in pairs(tableA) do
			if not self:AreTablesEqual(v, tableB[k]) then return false end
		end
...
function Clockwork.kernel:IsDefaultWeapon(weapon)
	if IsValid(weapon) then
		return DEFAULT_WEAPONS[weapon:GetClass()]
	end
...
function LIBRARY:AddToMetaTable(metaName, funcName, newName)
	...
	metaTable[newName or funcName] = function(...) return self[funcName](self, ...) end
end
...
function Clockwork.kernel:NewLibrary(libName)
	if not Clockwork.Libraries[libName] then
...
function Clockwork.kernel:IsVowel(letter)
	return VOWELS[string.lower(letter)]
end
...
function Clockwork.kernel:GetAmmoInformation(weapon)
	if IsValid(weapon) and IsValid(weapon.Owner) and weapon.Primary and weapon.Secondary then
		...
		if not weapon.AmmoInfo.primary.doesNotShoot and weapon.AmmoInfo.primary.ownerAmmo > 0 then
		...
		if not weapon.AmmoInfo.secondary.doesNotShoot and weapon.AmmoInfo.secondary.ownerAmmo > 0 then
...
function Clockwork:PlayerFootstep(player, position, foot, sound, volume, recipientFilter)
	...
	if not self.plugin:Call("PrePlayerDefaultFootstep", player, position, foot, sound, volume, recipientFilter) then
		local itemTable = player:GetClothesItem()

		if itemTable then
			if player:IsRunning() or player:IsJogging() then
				if itemTable.runSound then
					if istable(itemTable.runSound) then
			...
			elseif itemTable.walkSound then
				if istable(itemTable.walkSound) then
...
function Clockwork:HandlePlayerJumping(player)
	if not player.m_bJumping and not player:OnGround() and player:WaterLevel() <= 0 then
...
function Clockwork:HandlePlayerSwimming(player)
	if player:WaterLevel() >= 2 then
		...
		if not player.m_bFirstSwimFrame then
...
function Clockwork:TranslateActivity(player, act)
	...
	if string.find(model, "/player/") then
		...
		if not bIsRaised or act == newAct then
...
function Clockwork:CalcMainActivity(player, velocity)
	...
	if not self:HandlePlayerDriving(player) and not self:HandlePlayerJumping(player) and not self:HandlePlayerDucking(player, velocity) and not self:HandlePlayerSwimming(player) and not self:HandlePlayerNoClipping(player, velocity) and not self:HandlePlayerVaulting(player, velocity) then
		local velLength = velocity:Length2D()
	...
	if isstring(player.CalcSeqOverride) then
		player.CalcSeqOverride = player:LookupSequence(player.CalcSeqOverride)
	end

	if isstring(player.CalcIdeal) then
...
function Clockwork:SetupMove(player, moveData)
	if SERVER and (player:Alive() and not player:IsRagdolled()) then
...
function Clockwork:DoAnimationEvent(player, event, data)
	local model = player:GetModel()
	if string.find(model, "/player/") then return self.BaseClass:DoAnimationEvent(player, event, data) end
	local weapon = player:GetActiveWeapon()
...
if SERVER then
	...
	function Clockwork.kernel:SaveSchemaData(fileName, data)
		...
		return Clockwork.file:Write("data/clockwork/data/schemas/" .. self:GetSchemaFolder() .. "/" .. fileName .. ".txt", self:Serialize(data))
	end
	...
	function Clockwork.kernel:DeleteSchemaData(fileName)
		return Clockwork.file:Delete("data/clockwork/data/schemas/" .. self:GetSchemaFolder() .. "/" .. fileName .. ".txt")
	end
	...
	function Clockwork.kernel:SchemaDataExists(fileName)
		return _file.Exists("data/clockwork/data/schemas/" .. self:GetSchemaFolder() .. "/" .. fileName .. ".txt", "GAME")
	end
	...
	function Clockwork.kernel:GetSchemaDataPath()
		return "data/clockwork/data/schemas/" .. self:GetSchemaFolder()
	end
	...
	function Clockwork.kernel:GetSchemaGamemodeName()
		return self:GetSchemaGamemodeInfo()["name"]
	end
	...
	function Clockwork.kernel:GetSchemaGamemodeVersion()
		return self:GetSchemaGamemodeInfo()["version"]
	end
	...
	function Clockwork.kernel:FindSchemaDataInDir(directory)
		return _file.Find("data/clockwork/data/schemas/" .. self:GetSchemaFolder() .. "/" .. directory, "GAME")
	end
	...
	function Clockwork.kernel:RestoreSchemaData(fileName, failSafe)
		if self:SchemaDataExists(fileName) then
			local data = Clockwork.file:Read("data/clockwork/data/schemas/" .. self:GetSchemaFolder() .. "/" .. fileName .. ".txt", "namedesc")
	...
	function Clockwork.kernel:ClockworkDataExists(fileName)
		return _file.Exists("clockwork/data/" .. fileName .. ".txt", "DATA")
	end
	...
	function Clockwork.kernel:DeleteClockworkData(fileName)
		return Clockwork.file:Delete("data/clockwork/data" .. fileName .. ".txt")
	end
	...
	function Clockwork.kernel:ConvertForce(force, limit)
		...
		if forceLength == 0 then return Vector(0, 0, 0) end
	...
	function Clockwork.kernel:HandleWeaponFireDelay(player, bIsRaised, weapon, curTime)
		...
		if not Clockwork.plugin:Call("PlayerCanFireWeapon", player, bIsRaised, weapon, true) then
			delaySecondaryFire = curTime + 60
		end

		if not Clockwork.plugin:Call("PlayerCanFireWeapon", player, bIsRaised, weapon) then
		...
		if delaySecondaryFire then
			if not weapon.secondaryFireDelayed then
		...
		if delayPrimaryFire then
			if not weapon.primaryFireDelayed then
	...
	function Clockwork:ScaleDamageByHitGroup(player, attacker, hitGroup, damageInfo, baseDamage)
		if not damageInfo:IsFallDamage() and not damageInfo:IsDamageType(DMG_CRUSH) then
		...
		for k, v in pairs(Clockwork.HitGroupBonesCache) do
			...
			if bone then
				...
				if bonePosition then
					...
					if (not closest[1] or distance < closest[1]) and (not minimum or distance <= minimum) then
						closest[1] = distance
						closest[2] = bone
					end
				end
		...
		for k, v in pairs(Clockwork.HitGroupBonesCache) do
			...
			if bone then
				...
				if position then
					...
					if not closest[1] or distance < closest[1] then
						closest[1] = distance
						closest[2] = v[2]
					end
				end
	...
	function Clockwork.kernel:CreateBloodEffects(position, decals, entity, forceVec, fScale)
		if not entity.cwNextBlood or CurTime() >= entity.cwNextBlood then
	...
	function Clockwork.kernel:DoEntityTakeDamageHook(entity, damageInfo)
		...
		if not hook.Call("PlayerShouldTakeDamage", Clockwork, player, attacker, inflictor, damageInfo) or player:IsInGodMode() then
	...
	function Clockwork.kernel:DistributeGeneratorCash()
		...
		for k, v in pairs(generatorEntities) do
			...
			if not (IsValid(player) and v:GetPower() ~= 0) then continue end
			...
			if not Clockwork.plugin:Call("PlayerCanEarnGeneratorCash", player, info, info.cash) then continue end
	...
	function Clockwork.kernel:ServerLog(text)
		...
		if dateInfo then
			if dateInfo.month < 10 then
				dateInfo.month = "0" .. dateInfo.month
			end

			if dateInfo.day < 10 then
				dateInfo.day = "0" .. dateInfo.day
			end

			local fileName = dateInfo.year .. "-" .. dateInfo.month .. "-" .. dateInfo.day

			if dateInfo.hour < 10 then
				dateInfo.hour = "0" .. dateInfo.hour
			end

			if dateInfo.min < 10 then
				dateInfo.min = "0" .. dateInfo.min
			end

			if dateInfo.sec < 10 then
				dateInfo.sec = "0" .. dateInfo.sec
			end

			local time = dateInfo.hour .. ":" .. dateInfo.min .. ":" .. dateInfo.sec
			local logText = time .. ": " .. string.gsub(text, "\n", "")
			Clockwork.file:Append("data/clockwork/logs/" .. fileName .. ".log", logText .. "\n")
		end
	...
	function Clockwork.kernel:RegisterNetworkProxy(entity, name, Callback)
		if not Clockwork.NetworkProxies[entity] then
	...
	function Clockwork.kernel:GetMaterial(materialPath, pngParameters)
		...
		if not self.CachedMaterial[materialPath] then
	...
	function Clockwork.kernel:CalculateAlphaFromDistance(maximum, start, finish)
		if isentity(start) then
			if start:IsPlayer() then
		...
		if isentity(finish) then
			if finish:IsPlayer() then
	...
	function Clockwork.kernel:WrapText(text, font, maximumWidth, baseTable)
		if maximumWidth <= 0 or not text or text == "" then return end
		...
		local menuPanel = self:AddMenuFromData(nil, options, function(panel, option, arguments)
			if itemTable and istable(arguments) and arguments.isOptionTable then
				panel:AddOption(T(arguments.title), function()
					if not itemTable.HandleOptions then return end
			...
			if istable(arguments) then
				if arguments.isOrdered then
					...
					table.insert(panel.Items, 1, panel)
				end
	...
	function Clockwork.kernel:AddMenuFromData(menuPanel, data, Callback, minimumWidth, manualOpen)
		...
		for k, v in pairs(options) do
			if istable(v[2]) and not v[2].isArgTable then
	...
	function Clockwork.kernel:AddCenterHint(text, delay, color, noSound, showDuplicated)
		...
		if not showDuplicated then
			for k, v in pairs(self.CenterHints) do
				if v.text == localized then return end
			end
		end
	...
	local function UpdateCenterHint(index, hintInfo, iCount)
		...
		local fontWidth, fontHeight = Clockwork.kernel:GetCachedTextSize(hintsFont, hintInfo.text)
	...
	local function Util_DrawText(info, text, color, bCentered, sFont)
		...
		if sFont then
			Clockwork.kernel:OverrideMainFont(sFont)
		end

		if not bCentered then
			info.y, realWidth = Clockwork.kernel:DrawInfo(text, info.x - info.width / 2, info.y, color, nil, true)
		else
			info.y, realWidth = Clockwork.kernel:DrawInfo(text, info.x, info.y, color)
		end
	...
	function Clockwork.kernel:DrawDateTime()
		...
		if Clockwork.LastDateTimeInfo and Clockwork.LastDateTimeInfo.y > info.y then
			...
			self:DrawInfo(L("CharacterRoleplayInfo"), x, y + 4, colorInfo, nil, true, function(x, y, width, height) return x, y - height end)
			...
			if self:CanCreateInfoMenuPanel() and self:IsInfoMenuOpen() then
				...
				if not Clockwork.InfoMenuPanel.VisibilitySet then
		...
		if not Clockwork.plugin:Call("PlayerCanSeeLimbDamage") then return end
		local tipHeight = 0
		local tipWidth = 0
		local limbInfo = {}
		local height = 240
	...
	function Clockwork.kernel:DrawBars(info, class)
		if Clockwork.plugin:Call("PlayerCanSeeBars", class) then
			...
			for k, v in pairs(Clockwork.bars.stored) do
				Clockwork.bars.y = self:DrawBar(Clockwork.bars.x, Clockwork.bars.y, Clockwork.bars.width, Clockwork.bars.height, v.color, v.text, v.value, v.maximum, v.flash, {
					uniqueID = v.uniqueID
				}) + (Clockwork.bars.padding + 2)
			end
	...
	function Clockwork.kernel:DrawBar(x, y, width, height, color, text, value, maximum, flash, barInfo)
		...
		if not Clockwork.plugin:Call("PreDrawBar", barInfo) then
			...
			local alpha = math.Clamp(math.sin(UnPredictedCurTime() * 50), 0, 50)
	...
	function Clockwork.kernel:OverrideMainFont(font)
		if font then
			if not Clockwork.PreviousMainFont then
	...
	function Clockwork.kernel:GetCachedTextSize(font, text)
		if not Clockwork.CachedTextSizes then
			Clockwork.CachedTextSizes = {}
		end

		if not Clockwork.CachedTextSizes[font] then
			Clockwork.CachedTextSizes[font] = {}
		end

		if not Clockwork.CachedTextSizes[font][text] then
	...
	function Clockwork.kernel:DrawPlayerInfo(info)
		if not Clockwork.plugin:Call("PlayerCanSeePlayerInfo") then return end
		local subInformation = Clockwork.PlayerInfoText.subText
		local information = Clockwork.PlayerInfoText.text
		local colorWhite = Clockwork.option:GetColor("white")
		local textWidth, textHeight = self:GetCachedTextSize(Clockwork.option:GetFont("player_info_text"), "U")
		...
		if #information == 0 and #subInformation == 0 then return end
		...
		if not Clockwork.plugin:Call("PreDrawPlayerInfo", boxInfo, information, subInformation) then
	...
	function Clockwork.kernel:GetRagdollEyeAngles()
		if not Clockwork.RagdollEyeAngles then
	...
	function Clockwork.kernel:DrawGradient(gradientType, x, y, width, height, color)
		if not Clockwork.Gradients[gradientType] then return end
	...
	function Clockwork.kernel:HandleItemSpawnIconClick(itemTable, spawnIcon, Callback)
		...
		if itemTable.OnUse and not itemTable.noInventoryUse then
		...
		if #itemFunctions == 0 and not Callback then return end
		local options = {}
		...
		for k, v in pairs(itemFunctions) do
			...
			elseif v == dropText then
				itemMenu:AddOption(T(v), function()
					if itemTable then
						self:RunCommand("InvAction", "drop", itemTable("uniqueID"), itemTable("itemID"))
					end
				end)
			elseif v == destroyText then
				...
				subMenu:AddOption("Yes", function()
					if itemTable then
						self:RunCommand("InvAction", "destroy", itemTable("uniqueID"), itemTable("itemID"))
					end
				end)
			...
			elseif type(v) == "table" then
				itemMenu:AddOption(T(v.title), function()
					...
					if defaultAction then
						self:RunCommand("InvAction", v.name, itemTable("uniqueID"), itemTable("itemID"))
					end
				end)
			else
				...
				itemMenu:AddOption(T(v), function()
					if itemTable then
						self:RunCommand("InvAction", v, itemTable("uniqueID"), itemTable("itemID"))
					end
				end)
			end
		end
	...
	function Clockwork.kernel:HandleItemSpawnIconRightClick(itemTable, spawnIcon)
		if itemTable.OnHandleRightClick then
			...
			if functionName and functionName ~= "Use" then
				...
				if (customFunctions and table.HasValue(customFunctions, functionName)) and itemTable.OnCustomFunction then
					itemTable:OnCustomFunction()
				end

				self:RunCommand("InvAction", string.lower(functionName), itemTable("uniqueID"), itemTable("itemID"))
	...
	function Clockwork.kernel:CreateMarkupToolTip(panel)
		...
		function panel.OnCursorEntered(pan, ...)
			if pan.OldCursorEntered then
				pan:OldCursorEntered(...)
			end

			Clockwork.MarkupToolTip = pan
		end

		function panel.OnCursorExited(pan, ...)
			if pan.OldCursorExited then
				pan:OldCursorExited(...)
			end

			if Clockwork.MarkupToolTip == pan then
				Clockwork.MarkupToolTip = nil
			end
		end

		function panel.SetMarkupToolTip(pan, text)
			if not string.find(text, "</font>") then
				text = Clockwork.kernel:MarkupTextWithColor(text)
			end

			if not pan.MarkupToolTip or pan.MarkupToolTip.text ~= text then
				pan.MarkupToolTip = {
					object = markup.Parse(text, ScrW() * 0.25),
					text = text
				}

				self:OverrideMarkupDraw(pan.MarkupToolTip.object)
			end
		end

		function panel.GetMarkupToolTip(pan)
	...
	function Clockwork.kernel:CreateCustomCategoryPanel(categoryName, parent)
		if not parent.CategoryList then
			parent.CategoryList = {}
		end
	...
	function Clockwork.kernel:GetNoticePanel()
		if IsValid(Clockwork.NoticePanel) and Clockwork.NoticePanel:IsVisible() then return Clockwork.NoticePanel end
	end
	...
	function Clockwork.kernel:IsUsingTool()
		if IsValid(Clockwork.Client:GetActiveWeapon()) and Clockwork.Client:GetActiveWeapon():GetClass() == "gmod_tool" then
			return true
		else
			return false
		end
	...
	function Clockwork.kernel:IsUsingCamera()
		if IsValid(Clockwork.Client:GetActiveWeapon()) and Clockwork.Client:GetActiveWeapon():GetClass() == "gmod_camera" then
			return true
		else
			return false
		end
	...
	function Clockwork.kernel:CalculateScreenFading()
		if Clockwork.plugin:Call("ShouldPlayerScreenFadeBlack") then
			if not Clockwork.BlackFadeIn then
	...
	function Clockwork.kernel:DrawCinematic(cinematicTable, curTime)
		...
		if cinematicTable.goBack and curTime > cinematicTable.goBack then
			...
			if cinematicTable.add == maxBarLength and not cinematicTable.goBack then
			...
			if Clockwork.CinematicScreenAlpha and Clockwork.CinematicScreenTarget then
				...
				if Clockwork.CinematicScreenAlpha == Clockwork.CinematicScreenTarget then
					if Clockwork.CinematicScreenTarget == 255 then
						if not Clockwork.CinematicScreenGoBack then
				...
				if not Clockwork.CinematicScreenDone and cinematicInfo.credits then
	...
	function Clockwork.kernel:DrawCinematicIntroBars()
		if Clockwork.config:Get("draw_intro_bars"):Get() then
			...
			if not Clockwork.CinematicBarsTarget and not Clockwork.CinematicBarsAlpha then
			...
			elseif Clockwork.CinematicScreenBarLength ~= maxBarLength then
				if not Clockwork.IntroBarsMultiplier then
	...
	function Clockwork.kernel:DrawCinematicInfo()
		if not Clockwork.CinematicInfoAlpha and not Clockwork.CinematicInfoSlide then
		...
		if cinematicInfo then
			...
			if cinematicInfo.title then
				...
				local _, textHeight = self:GetCachedTextSize(introTextBigFont, cinematicInfoTitle)
				local boxAlpha = math.min(Clockwork.CinematicInfoAlpha, 150)

				if cinematicInfo.text then
					local _, smallTextHeight = self:GetCachedTextSize(introTextSmallFont, cinematicIntroText)
					self:DrawGradient(GRADIENT_RIGHT, 0, textPosY - 80, screenWidth, textHeight + smallTextHeight + 160, Color(100, 100, 100, boxAlpha))
				else
					self:DrawGradient(GRADIENT_RIGHT, 0, textPosY - 80, screenWidth, textHeight + 160, Color(100, 100, 100, boxAlpha))
				end
	...
	function Clockwork.kernel:IsCharacterScreenOpen(isVisible)
		if Clockwork.character:IsPanelOpen() then
			...
			if isVisible then
				if panel then return panel:IsVisible() end
			else
				return panel ~= nil
			end
		end
...
function Clockwork.kernel:ModifyPhysDesc(description)
	if string.utf8len(description) <= 256 then
		if not string.find(string.utf8sub(description, -2), "%p") then
			return description .. "."
		else
			return description
		end
...
function Clockwork.kernel:IncludePrefixed(fileName)
	...
	if isServer and not SERVER then return end
...
function Clockwork.kernel:IncludePlugins(directory, bFromBase)
	...
	local _, pluginFolders = _file.Find(directory .. "*", "LUA", "namedesc")
...
function Clockwork.kernel:CallTimerThink(curTime)
	for k, v in pairs(Clockwork.Timers) do
		if not v.paused and curTime >= v.nextCall then
...
function Clockwork.kernel:PauseTimer(name)
	if Clockwork.Timers[name] and not Clockwork.Timers[name].paused then
...
function Clockwork.kernel:HasObjectAccess(player, object)
	...
	if not object.access and not object.factions and not object.classes then
		hasAccess = true
	end
	...
	if object.HasObjectAccess then return object:HasObjectAccess(player, hasAccess) end
...
function Clockwork.kernel:ValidateTableKeys(baseTable)
	...
	for i = 1, #baseTable do
		if not baseTable[i] then
			table.insert(invalidKeys, i)
		end
	end

	for i = #invalidKeys, 1, -1 do
		table.remove(baseTable, invalidKeys[i])
	end
end
...
function Clockwork.kernel:SetSharedVar(key, value, sharedTable)
	if not sharedTable then
		...
		if SERVER then
			Clockwork.datastream:Start(nil, "SetSharedTableVar", {
...
function Clockwork.kernel:GetSharedVar(key, sharedTable)
	if not sharedTable then
		...
		if sharedVars and sharedVars[key] then
			local class = self:ConvertNetworkedClass(sharedVars[key].class)
			if class then return _G["GetGlobal" .. class](key) end
		end
	else
		sharedTable = Clockwork.SharedTables[sharedTable]
		if sharedTable then return sharedTable[key] end
	end
end
